-- FEATURE 012.1: rejeitar NFe de marketplace (EBAZAR / Mercado Livre)
-- CNPJ raiz 03.007.331/* não é fornecedor de reposição de estoque.

CREATE OR REPLACE FUNCTION public.fz_is_marketplace_supplier(p_document text)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT coalesce(
    left(public.fz_normalize_digits(p_document), 8) IN (
      '03007331'  -- EBAZAR.COM.BR LTDA (Mercado Livre)
    ),
    false
  );
$$;

COMMENT ON FUNCTION public.fz_is_marketplace_supplier(text) IS
  'FEATURE 012.1: CNPJ de marketplace (ex. EBAZAR/ML) — não importar como compra de estoque.';
...
GRANT EXECUTE ON FUNCTION public.fz_is_marketplace_supplier(text) TO service_role;
