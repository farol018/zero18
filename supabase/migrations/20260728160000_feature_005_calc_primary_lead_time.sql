-- FEATURE 005: cálculo usa purchase_multiple e lead_time do primary
-- Prefere product_suppliers (is_primary + is_active); fallback products.*
-- lead_time null/ausente = 0 → comportamento idêntico ao anterior
-- Recria views dependentes preservando dual-read da FEATURE 004
...
calc AS (
  SELECT
    ...
    GREATEST(
      COALESCE(ps.purchase_multiple, p.purchase_multiple, 1),
      1
    )::numeric AS purchase_multiple,
    sb.estoque_atual,
    ...
    CASE
      ...
      WHEN sb.estoque_atual < 0 THEN
        (cs.coverage_days::numeric + COALESCE(ps.lead_time_days, 0)::numeric)
          * (c.total_saida / NULLIF(cs.consumption_window_days, 0)::numeric)
        + abs(sb.estoque_atual)
      WHEN sb.estoque_atual = 0 THEN
        (cs.coverage_days::numeric + COALESCE(ps.lead_time_days, 0)::numeric)
          * (c.total_saida / NULLIF(cs.consumption_window_days, 0)::numeric)
      WHEN (sb.estoque_atual / NULLIF(c.total_saida / NULLIF(cs.consumption_window_days, 0)::numeric, 0))
           >= (cs.coverage_days::numeric + COALESCE(ps.lead_time_days, 0)::numeric)
        THEN 0::numeric
      ELSE (
             (cs.coverage_days::numeric + COALESCE(ps.lead_time_days, 0)::numeric)
             - sb.estoque_atual / NULLIF(c.total_saida / NULLIF(cs.consumption_window_days, 0)::numeric, 0)
           )
           * (c.total_saida / NULLIF(cs.consumption_window_days, 0)::numeric)
  ...
  LEFT JOIN public.product_suppliers ps
   ...
   AND ps.company_id = sb.company_id
...
-- FEATURE 004 dual-read preservado
...
COMMENT ON VIEW public.stock_analysis IS
  'FEATURE 005: purchase_multiple/cost/lead_time do primary; lead_time null = legado';
